...
/*   Updated: 2021/04/16 18:12:33 by pantigon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(cub->sprt_plc = malloc(sizeof(t_plr) * cub->num_sprite)))
		ft_notify_error("fail sprite", cub);
	if (!(cub->rc.sp_order = malloc(sizeof(int) * cub->num_sprite)))
		ft_notify_error("fail sprite", cub);
	if (!(cub->rc.sp_distance = malloc(sizeof(double) * cub->num_sprite)))
		ft_notify_error("fail sprite", cub);
	ft_point_sprt(cub);
	ft_chain_sprt(cub);
	if (cub->sprt_plc)
