...
// {
// 	if (cub->rc.text.id == 0)
// 		pixel_textur(cub->textur3, cub);
// 	else if (cub->rc.text.id == 1)
// 		pixel_textur(cub->textur4, cub);
// 	else if (cub->rc.text.id == 2)
// 		pixel_textur(cub->textur1, cub);
...
void	ft_get_pixel(t_cub *cub, int x)
{
	...
	y = 0;
	while (y < cub->rc.wstart)
		ft_get_pixel_col(cub, cub->col.c, x, y++);
	while (y >= cub->rc.wstart && y <= cub->rc.wend)
	{
		cub->rc.text.y = (int)cub->rc.text_pos & (64 - 1);
		cub->rc.text_pos += cub->rc.step_text;
		ft_get_col_text(cub);
		ft_get_pixel_col(cub->img, cub->col.text, x, y);
		y++;
	}
	while (y < cub->win_h)
		ft_get_pixel_col(cub->img, cub->col.f, x, y++);
}
