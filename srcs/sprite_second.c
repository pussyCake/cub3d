...
/*   Updated: 2021/04/24 21:12:29 by pantigon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sprt_for_loop(t_cub *cub)
{
		...
		<= cub->win_w && cub->sprt.tf_y < cub->rc.zbuff[cub->sprt.column - 1])
	{
		...
		{
			d = y * 2 - cub->win_h * 1 + cub->sprt.h * 1;
			cub->sprt.y = ((d * cub->text_sp->height) / cub->sprt.h) / 2;
...
void	ft_for_draw_sprt(t_cub *cub)
{
	...
	cub->sprt.w = abs((int)(cub->win_w / 1.6 / (cub->sprt.tf_y)));
...
void	ft_sprtcast(t_cub *cub, int i)
{
	...
	cub->sprt.scrnx = (int)((cub->win_w / 2) * (1 + cub->sprt.tf_x
				/ cub->sprt.tf_y));
	cub->sprt.h = abs((int)(cub->win_w / 1.6 / cub->sprt.tf_y));
...
void	ft_spr_crt(t_cub *cub)
{
	...
	while (++i < cub->num_sprite)
	{
		...
		{
			cub->sprt.x = (int)(2
					* (cub->sprt.column - (-cub->sprt.w / 2.0
							+ cub->sprt.scrnx))
					* cub->text_sp->width / cub->sprt.w / 2);
